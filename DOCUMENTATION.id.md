...
Versi dipin via [Volta](https://volta.sh) di `package.json` (`volta`, `engines`, `packageManager`):
...
Panduan setup toolchain lengkap (macOS / Linux / Windows, dengan atau tanpa Volta): [VOLTA.id.md](./VOLTA.id.md) ([English](./VOLTA.md)).

### Dependensi runtime

| Paket                 | Versi  | Peran                              |
| --------------------- | ------ | ---------------------------------- |
| `react` + `react-dom` | 19.2.8 | Rendering UI                       |
...
Setup toolchain (disarankan): lihat [VOLTA.id.md](./VOLTA.id.md). Jika Volta sudah terpasang:
...
Katalog live menampilkan props tiap komponen, preview state × theme, demo interaktif (termasuk console untuk `console.log`), kode yang bisa diedit dan memperbarui preview, serta contoh siap salin untuk setiap komponen bersama.
...
Ukuran: `sm` · `md` · `lg`  
Juga mendukung `fullWidth`, `disabled`, `className`, dan atribut button native.
...
Mode data: `options` · `string[]` items · object JSON · model bertipe. Lihat katalog live untuk demo searchable, addOption, dan custom label.

### Contoh penggunaan: DataTable + Pagination
...
Untuk **halaman baru** (mis. Inventaris), tambahkan tipe response, tipe domain, mapper, dan endpoint. Lihat [Bagian 15: Langkah 7](#id-15-buat-halaman-baru--manual).
...
Tip: `make feature` mempertahankan spasi di `name` / label seperti yang Anda ketik untuk tampilan, dan merapikan separator saat menurunkan feature key serta nama folder.
