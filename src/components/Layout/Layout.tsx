...
export function Divider({
  ...
  const hasLabel = children != null && String(children).trim().length > 0;
  ...
  if (orientation === 'vertical') {
    const verticalPos = dividerVerticalLabelPositionClasses[labelPosition];
    return (
      <div
        role="separator"
        aria-orientation="vertical"
        className={cn(
          'flex h-full min-h-[inherit] shrink-0 flex-col items-center gap-2 self-stretch text-sm text-gray-500 dark:text-gray-400',
          plain ? 'mx-0 py-0' : 'mx-3 py-2',
          className,
        )}
        {...rest}
      >
        <span
          className={cn(
            'min-h-[8px] w-px border-l border-gray-200 dark:border-gray-700',
            borderClass,
            verticalPos.top,
          )}
          aria-hidden="true"
        />
        <span className="shrink-0 px-2 text-center text-xs font-medium leading-tight">
          {children}
        </span>
        <span
          className={cn(
