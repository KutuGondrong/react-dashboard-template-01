...
function pickLegendLayout(args: {
  ...
  const { box, legend, definiteHeight, sizeProp, legendPosition = 'auto' } = args;
  ...
  if (legendPosition === 'center') {
    const side = Math.min(box.width, box.height);
    return { layout: 'center', ringSize: clampRingSize(side, sizeProp, side) };
  }
  if (legendPosition === 'right') {
    const widthForRing = Math.max(0, box.width - LAYOUT_GAP - legend.width);
    const heightBudget = definiteHeight ? box.height : sizeTarget;
    return {
      layout: 'right',
      ringSize: clampRingSize(widthForRing, sizeProp, heightBudget),
    };
  }
...
export function DonutChart({
  ...
  }, [size, segments, total, legendPosition]);
