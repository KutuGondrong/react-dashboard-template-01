...
This project pins **Node.js `24.18.0`** and **pnpm `11.17.0`** in `package.json` (`volta`, `engines`, `packageManager`). Prefer the same versions with or without Volta.
...
**Toolchain setup (macOS / Linux / Windows, with or without Volta):** see [VOLTA.md](./VOLTA.md) ([Bahasa Indonesia](./VOLTA.id.md)).

Quick path if you already use Volta (enable `VOLTA_FEATURE_PNPM=1` first, details in VOLTA.md):
...
| Wrong Node / pnpm version        | See [VOLTA.md](./VOLTA.md); check `which node` is under `~/.volta/bin`           |
| `node -v` still nvm / old Node   | Put `~/.volta/bin` first on `PATH` (nvm often shadows Volta; see VOLTA.md)       |
| `pnpm: command not found`        | Enable `VOLTA_FEATURE_PNPM=1`, then `volta install pnpm@11.17.0` (see VOLTA.md)  |
...
- [VOLTA.md](./VOLTA.md): Node.js / pnpm toolchain setup (English)
- [VOLTA.id.md](./VOLTA.id.md): setup toolchain Node.js / pnpm (Bahasa Indonesia)
