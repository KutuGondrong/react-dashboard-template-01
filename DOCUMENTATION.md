...
Pinned via [Volta](https://volta.sh) in `package.json` (`volta`, `engines`, `packageManager`):
...
Full toolchain setup (macOS / Linux / Windows, with or without Volta): [VOLTA.md](./VOLTA.md) ([Bahasa Indonesia](./VOLTA.id.md)).
...
Toolchain setup (recommended): see [VOLTA.md](./VOLTA.md). If you already use Volta:
...
Loads global styles (`index.css`) and renders `<App />` inside React 19 `StrictMode`.
...
The live catalog shows each component’s props, state × theme previews, interactive demos (including a console for `console.log`), editable code that updates the live preview, and copy-paste samples for every shared component.
...
<Button variant="primary" size="md" isLoading={saving} onClick={onClick}>
  Save
</Button>;
```
...
Sizes: `sm` · `md` · `lg`  
Also supports `fullWidth`, `disabled`, `className`, and native button attributes.

### Usage example: ComboBox
...
<ComboBox label="Country" placeholder="Select option" options={options} onSelect={onSelect} />;
```

Data modes: `options` · `string[]` items · JSON objects · typed model. See the live catalog for searchable, addOption, and custom label demos.
...
For a **new page you create from scratch** (e.g. Inventory), add response types, domain types, mappers, and endpoints following [Section 15: Step 11](#step-11--connect-real-api-when-backend-is-ready).
...
Tip: `make feature` keeps spaces in your `name` / labels as you typed them for display, and collapses separators when deriving the feature key and folder name.
