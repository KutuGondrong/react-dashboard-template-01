...
**Panduan setup lengkap (macOS / Linux / Windows, dengan atau tanpa Volta):** [VOLTA.id.md](./VOLTA.id.md) ([English](./VOLTA.md)).
...
Konfigurasi pin tercatat eksplisit di `package.json` (`volta`, `engines`, `packageManager`):
...
Ringkasan cepat jika Volta sudah terpasang (aktifkan `VOLTA_FEATURE_PNPM=1` dulu; detail di [VOLTA.id.md](./VOLTA.id.md)):
...
> Detail per OS, `VOLTA_FEATURE_PNPM`, troubleshooting, dan opsi tanpa Volta ada di **[VOLTA.id.md](./VOLTA.id.md)**. Ringkasan singkat di bawah.
...
# Restart shell, lalu install versi
volta install node@24.18.0 pnpm@11.17.0
```
...
2. Jalankan `volta install node@24.18.0 pnpm@11.17.0` jika:
   - Anda fork/clone repo dan versi aktif belum sesuai
   - Versi yang aktif tidak sesuai saat `node --version`
   - Detail flag `VOLTA_FEATURE_PNPM`: lihat [VOLTA.id.md](./VOLTA.id.md)
...
| ------------ | ------------------------------------------ | -------------------------- | ---------------------------------------- |
| UI Framework | `react` + `react-dom`                      | 19.2.8                     | Rendering, concurrent features, Suspense |
...
#### 3.2 React 19.2 + TypeScript 5.7 + Vite 5.4: Kombinasi Mutakhir

**React 19.2: mengapa versi ini?**
...
- **Actions & form hooks**: pola async update state yang lebih sederhana (`useActionState`, `useOptimistic`, `useFormStatus`) tersedia di React 19.
...
- **`ref` sebagai prop**: tidak wajib `forwardRef` untuk komponen function biasa (masih dipakai di beberapa komponen shared untuk kompatibilitas).
...
│   │   ├── components/            # Sidebar, SidebarIcons
...
```bash
which node          # harus .../.volta/bin/node (bukan ~/.nvm/...)
volta which node
volta install node@24.18.0 pnpm@11.17.0
# jika masih versi nvm: export PATH="$HOME/.volta/bin:$PATH" lalu hash -r
# detail: VOLTA.id.md (PATH / nvm shadow)
node --version  # harus v24.18.0
pnpm --version  # harus 11.17.0
```
...
# Pastikan VOLTA_FEATURE_PNPM=1 (lihat VOLTA.id.md)
